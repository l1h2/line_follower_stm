...
static float left_current_distance = 0.0f;
static float right_current_distance = 0.0f;
...
static inline void update_distances(void) {
    left_current_distance = ((float)left_encoder * CM_PER_PULSE);
    right_current_distance = ((float)right_encoder * CM_PER_PULSE);
...
static inline void update_speeds(void) {
    float interval_s = (current_time - encoder_data.last_update_time) / 1000.0f;
    ...
    encoder_data.left_speed = left_current_distance / interval_s;
    encoder_data.right_speed = right_current_distance / interval_s;
    encoder_data.speed =
...
void clear_encoder_data(void) {
    left_encoder = 0;
    right_encoder = 0;
    left_current_distance = 0.0f;
    right_current_distance = 0.0f;
